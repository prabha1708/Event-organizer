...
const app = http.createServer((req, res) => {
   if (req.url === "/reg" && req.method === "GET") {
      ...
      req.on("end", () => {
         ...
         const eventdate = newData.get("event-date");  // Updated field name
         const eventname = newData.get("event-name");  // Updated field name

         // Validate if fields are not empty
         if (!name || !email || !phone || !eventdate || !eventname) {
            // Redirect back to the registration page with an error message if validation fails
            res.writeHead(400, { "Location": "/reg" });
            return res.end("All fields are required");
         }

         // Prepare the registration data to store in JSON
         const userData = { name, email, phone, eventname, eventdate };
